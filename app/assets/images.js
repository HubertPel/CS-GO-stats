const images = {
  horizontalMaps: {
    boyard: require('./maps-horizontal/boyard.jpg'),
    chalice: require('./maps-horizontal/chalice.jpg'),
    vertigo: require('./maps-horizontal/vertigo.png'),
    inferno: require('./maps-horizontal/inferno.png'),
    overpass: require('./maps-horizontal/overpass.png'),
    cobblestone: require('./maps-horizontal/cobblestone.png'),
    train: require('./maps-horizontal/train.png'),
    nuke: require('./maps-horizontal/nuke.png'),
    shortdust: require('./maps-horizontal/shortdust.png'),
    lake: require('./maps-horizontal/lake.png'),
  },
  mapIcons: {
    boyard: require('./maps-icons/boyard_icon.png'),
    chalice: require('./maps-icons/chalice_icon.png'),
    vertigo: require('./maps-icons/vertigo_icon.png'),
    inferno: require('./maps-icons/inferno_icon.png'),
    overpass: require('./maps-icons/overpass_icon.png'),
    cobblestone: require('./maps-icons/cobblestone_icon.png'),
    train: require('./maps-icons/train_icon.png'),
    nuke: require('./maps-icons/nuke_icon.png'),
    shortdust: require('./maps-icons/shortdust_icon.png'),
    lake: require('./maps-icons/lake_icon.png'),
  },
  icons: {
    check: require('./icons/check.png'),
    check_white: require('./icons/check_white.png'),
    equal_white: require('./icons/equal_white.png'),
    x_white: require('./icons/x_white.png'),
    two_arrows_up_white: require('./icons/two_arrows_up_white.png'),
    two_arrows_down_white: require('./icons/two_arrows_down_white.png'),
  },
};
export default images;
